...
class ListProjectCubit extends Cubit<ListProjectCubitState> {
  ...
  Future<void> searchProject(String valueSearch) async {
    emit(ListProjectCubitIsLoading());
    currentPage = 0;
    hasMoreData = true;
    listProjects.clear();
    ...
    try {
      final fetchData = await fetchProject(valueSearch, currentPage);
      listProjects = fetchData!.result.projects;
      hasMoreData = listProjects.length == pageSize;
      emit(ListProjectCubitIsSuccess(
          listProjects, fetchData.result.paging.totalPage, hasMoreData));
    } catch (e) {
  ...
  Future<void> loadMoreItems(String query) async {
    emit(ListProjectCubitLoadMore(listProjects));
    currentPage++;
    try {
      ...
      emit(ListProjectCubitIsSuccess(
          listProjects, result.result.paging.totalPage, hasMoreData));
    } catch (error) {
